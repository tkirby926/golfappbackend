...
def get_search_friends(user):
    ...
    results = cursor.fetchall()
    index = len(results)
    cursor = run_query(connection, "SELECT username, firstname, lastname, imageurl FROM USERS U WHERE username NOT IN (SELECT username FROM USERS U, Friendships F WHERE ((F.userid2 = %s AND U.Username = F.userid1) OR (F.userid1 = %s AND U.Username = F.userid2))) ORDER BY RAND() LIMIT %s;", (user, user, 8 - len(results)))
